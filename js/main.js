...
function getMediaPosters(selectedGenre) {
  ...
  if (selectedGenre == 'All') {
    ...
    mediaCarouselMovies = document.getElementById("myfilm");
  }
    ...
    if (selectedGenre == 'Action') {
        ...
        mediaCarouselMovies = document.getElementsByClassName("myfilm-card");
      }
...
function updateResults() {
  ...
  for (var name = 0; name < movieTitle.length; name++) {
    ...
    if (compareMovieTitles.indexOf(query) === -1) {
      movieTitle[name].closest('div.myfilm-card').style.display = "none";
    } else {
      movieTitle[name].closest('div.myfilm-card').style.display = "block";
      counter++;
    }
  }
}
